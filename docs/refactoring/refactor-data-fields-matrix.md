...
| Matrix | Kernwerte | Einzelwert, Kalkulationsfeld, Vergleichswert | `sr6_matrix_*` | Issue-56-Pruefung abgeschlossen: ASDF und Overwatch bleiben manuelle Matrix-/Geraetewerte; Matrix-Initiative, Angriffswert, Verteidigungswert und Matrix-Widerstandswerte werden abgeleitet |
...
| Rigging | Kernwerte | Einzelwert, Kalkulationsfeld, Vergleichswert | `sr6_rigging_*` | Issue-56-Pruefung abgeschlossen: Riggerkonsolenwerte bleiben manuelle Geraetewerte; Rigging-Initiative, Angriffswert, Verteidigungswert und Matrix-Widerstandswerte werden abgeleitet |
...
| `Nahkampfwaffen: Angriffswert` | Kontextwert / Waffen-Basiswert | Waffen-Angriffswert pro Reichweite; die eingetragenen Werte sind vom Spieler gepflegte Basiswerte | `repeating_sr6nahkampfwaffen_*` | Die Waffe fuehrt zusaetzlich `Schadenstyp` und das verwendete `Attribut`; das Rolltemplate leitet daraus den endgueltigen Angriffswert ab |
...
- reichweitenabhaengige Waffen-`Angriffswerte` bleiben **Waffen-Kontextwerte**; Nahkampf-Reichweitenwerte sind Spieler-Basiswerte fuer die Rolltemplate-Ableitung
...
| Matrix-/Rigging-Kernwerte | Matrixattribute/Geraetewerte sind teils manuelle Werte; Initiative nutzt Modus-Sonderlogik | Issue-56-Pruefung abgeschlossen: ASDF-/Riggerkonsolenwerte bleiben manuell, Initiative ist berechnet, Angriffs-/Verteidigungswerte und Matrix-Widerstandswerte sind abgeleitet | Beibehalten; keine pauschale Dreiklang-Umstellung |
...
### Issue-56 Pruefbefund Matrix-/Rigging-Kernwerte

Matrix- und Rigging-Kernwerte enthalten bewusst mehr manuelle Geraetewerte als klassische `Attribut + Fertigkeit`-Proben. ASDF-Werte, Riggerkonsolenwerte und Overwatch bleiben manuelle Einzelwerte. Angriffswert, Verteidigungswert und die Matrix-Widerstandswerte werden aus diesen Geraetewerten und Charakterattributen abgeleitet. Gewuerfelt werden nur echte Proben-/Widerstandswerte; Initiative bleibt ein eigener Sonderwurf.

| Bereich | Feld | Soll-Logik | Sheet-Quelle | Status / Hinweis |
| --- | --- | --- | --- | --- |
| Matrix | `Angriff`, `Schleicher` | Aktive Cyberdeck-Werte der Persona, jeweils Basis + Modifikator | `sr6_matrix_angriff`, `sr6_matrix_angriff_modifikator`, `sr6_matrix_angriff_gesamtwert`, `sr6_matrix_schleicher`, `sr6_matrix_schleicher_modifikator`, `sr6_matrix_schleicher_gesamtwert` | Cyberdeck-Block; Basis/Mod bleiben manuell gefuehrt, Gesamtwert dient als Rollpool |
| Matrix | `Datenverarbeitung`, `Firewall` | Aktive Cyberbuchse-/Kommlink-Werte der Persona, jeweils Basis + Modifikator | `sr6_matrix_datenverarbeitung`, `sr6_matrix_datenverarbeitung_modifikator`, `sr6_matrix_datenverarbeitung_gesamtwert`, `sr6_matrix_firewall`, `sr6_matrix_firewall_modifikator`, `sr6_matrix_firewall_gesamtwert` | Cyberbuchse-/Kommlink-Block; Basis/Mod bleiben manuell gefuehrt, Gesamtwert dient als Rollpool |
| Matrix | `Cyberbuchse / Kommlink` | Aktive D/F-Quelle der Persona; Status, Stufe und VR-Bonuswuerfel sind Geraetekontext | `sr6_matrix_persona_status`, `sr6_matrix_persona_stufe`, `sr6_matrix_cyberbuchse_initiative_w6` | Keine automatische Uebernahme aus der Geraete-Liste; Kernwerte sind die aktive Persona-Konfiguration |
| Matrix | `Cyberdeck` | Aktive A/S-Quelle der Persona; Name, Geraetestufe und Programmslots sind Geraetekontext | `sr6_matrix_cyberdeck_name`, `sr6_matrix_geraetestufe`, `sr6_matrix_geraetestufe_modifikator`, `sr6_matrix_angriff`, `sr6_matrix_schleicher`, `sr6_matrix_programmslots` | Keine automatische Uebernahme aus der Geraete-Liste |
| Matrix | `Overwatch` | Manuell gefuehrter Overwatch-Wert | `sr6_matrix_overwatch` | Kein Standard-Wuerfelpool |
| Matrix | `Matrix-Initiative` | AR: wie physische Initiative (`Reaktion + Intuition` plus physische Initiative-W6); VR: `Datenverarbeitung + Intuition + 2W6/3W6 + VR-Bonuswuerfel` | `sr6_matrix_modus`, `sr6_matrix_datenverarbeitung`, `sr6_initiative_physisch_w6`, `sr6_matrix_cyberbuchse_initiative_w6`, Attribute `Reaktion`, `Intuition` | VR-Bonuswuerfel gelten nur bei `VR Kalt` / `VR Heiss` |
| Matrix | `Angriffswert`, `Verteidigungswert` | `Angriff + Schleicher`; `Datenverarbeitung + Firewall` | `sr6_matrix_angriffswert`, `sr6_matrix_verteidigungswert` | Berechnete Vergleichswerte ohne Rollbutton |
| Matrix | `Matrix Verteidigung`, `Matrix Schadenswiderstand`, `Biofeedback Schadenswiderstand` | `Intuition + Firewall`; `Firewall`; `Willenskraft` | `sr6_matrix_verteidigung`, `sr6_matrix_schadenswiderstand`, `sr6_matrix_biofeedback_schadenswiderstand` | Berechnet und rollbar als Sonderproben |
...
| Rigging | `Angriffswert`, `Verteidigungswert` | `Angriff + Schleicher`; `Datenverarbeitung + Firewall` der Riggerkonsole | `sr6_rigging_angriffswert`, `sr6_rigging_verteidigungswert` | Berechnete Vergleichswerte ohne Rollbutton |
| Rigging | `Matrix Verteidigung`, `Matrix Schadenswiderstand`, `Biofeedback Schadenswiderstand` | `Intuition + Firewall`; `Firewall`; `Willenskraft` mit Riggerkonsolen-Firewall | `sr6_rigging_matrix_verteidigung`, `sr6_rigging_matrix_schadenswiderstand`, `sr6_rigging_biofeedback_schadenswiderstand` | Berechnet und rollbar als Sonderproben |
